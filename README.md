...
- ROS non-default dependencies for [mesh_rviz_plugins](https://github.com/MIT-SPARK/mesh_rviz_plugins) (change `humble` for your ROS distribution):
```bash
sudo apt-get install ros-humble-image-geometry ros-humble-pcl-ros ros-humble-cv-bridge
```
...
mkdir -p ~/kimera_ros2_ws/src
cd ~/kimera_ros2_ws/
colcon init
colcon config -a --cmake-args -DGTSAM_USE_SYSTEM_EIGEN=ON -DCMAKE_BUILD_TYPE=RelWithDebInfo -DGTSAM_TANGENT_PREINTEGRATION=OFF -DGTSAM_BUILD_WITH_MARCH_NATIVE=OFF -DOPENGV_BUILD_WITH_MARCH_NATIVE=OFF
colcon config --merge-devel

# Clone the repo
cd ~/kimera_vio_ws/src
git clone git@github.com:ArghyaChatterjee/Kimera-VIO-ROS2.git
...
# Refresh workspace
source ~/kimera_ros2_ws/install/setup.bash
```
...
## Online
  ...
  > Note that you will need to both source ROS and your `colcon_ws` for each new terminal unless you added the following lines to your `~/.bashrc` file:
  > ```bash
  > source /opt/ros/humble/setup.bash  # Change `noetic` for your ROS distribution.
  > source ~/colcon_ws/devel/setup.bash # Change `bash` to the shell you use.
  > ```
